...
Ext.define('Ext.ux.panel.UploadPanel',
    {
        extend: 'Ext.grid.Panel', alias: 'widget.xuploadpanel'

// Configuration
        , title: 'Unggah', url: 'data/upload.jsp'	// URL to your server-side upload-script
        ...
        , texts: {
        status: ['Queued', 'Uploading', 'Unknown', 'Failed', 'Done'], DragDropAvailable: 'Drag&drop files here', noDragDropAvailable: 'This Browser doesn\'t support drag&drop.', emptyTextTpl: '<div style="color:#808080; margin:0 auto; text-align:center; top:48%; position:relative;">{0}</div>', cols: ["File", "Size", "State", "Mesage"], addButtonText: 'Add file', uploadButtonText: 'Upload', cancelButtonText: 'Cancel', deleteButtonText: 'Delete', deleteUploadedText: 'Delete finished', deleteAllText: 'Delete all', deleteSelectedText: 'Delete selected', progressCurrentFile: 'Current file:', progressTotal: 'Total:', statusInvalidSizeText: 'File size is to big', statusInvalidExtensionText: 'Invalid file-type'
    }
...
// Internal (do not change)
        ...
        , loadedFile: 0, constructor: function (config) {
        // List of files
        this.success = [];
        this.failed = [];
...
//        if (!Ext.ModelManager.getModel('Plupload')) {
            Ext.define('Plupload',
                {
                    extend: 'Ext.data.Model', fields: [ 'id', 'loaded', 'name', 'size', 'percent', 'status', 'msg' ]
                });
//        }

        config.store =
        {
            type: 'json', model: 'Plupload', listeners: {
            load: this.onStoreLoad, remove: this.onStoreRemove, update: this.onStoreUpdate, scope: this
        }, proxy: 'memory'
        };
        ...
        this.tbar =
        {
            ...
            items: [
                new Ext.Button({
                    text: this.texts.addButtonText, itemId: 'addButton', iconCls: config.addButtonCls || 'pluploadAddCls', disabled: true
                })
                , new Ext.Button({
                    text: this.texts.uploadButtonText, handler: this.onStart, scope: this, disabled: true, itemId: 'upload', iconCls: config.uploadButtonCls || 'pluploadUploadCls'
                })
                , new Ext.Button({
                    text: this.texts.cancelButtonText, handler: this.onCancel, scope: this, disabled: true, itemId: 'cancel', iconCls: config.cancelButtonCls || 'pluploadCancelCls'
                })
//                , new Ext.SplitButton({
//                    text: this.texts.deleteButtonText, handler: this.onDeleteSelected, menu: new Ext.menu.Menu({
//                        items: [
//                            {
...
//                            }
//                        ]
//                    }), scope: this, disabled: true, itemId: 'delete', iconCls: config.deleteButtonCls || 'pluploadDeleteCls'
//                })
            ]
        };
        ...
        this.bbar =
        {
            layout: 'hbox', style: { paddingLeft: '5px' }, items: [
            this.texts.progressCurrentFile
            , this.progressBarSingle
            , {
                xtype: 'tbtext', itemId: 'single', style: 'text-align:right', text: '', width: 100
            }
            , this.texts.progressTotal
            , this.progressBarAll
            , {
                xtype: 'tbtext', itemId: 'all', style: 'text-align:right', text: '', width: 100
            }
            , {
                xtype: 'tbtext', itemId: 'speed', style: 'text-align:right', text: '', width: 100
            }
            , {
                xtype: 'tbtext', itemId: 'remaining', style: 'text-align:right', text: '', width: 100
            }
        ]
        };
